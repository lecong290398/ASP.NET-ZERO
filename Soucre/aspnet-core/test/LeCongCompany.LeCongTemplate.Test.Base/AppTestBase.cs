...
using LeCongCompany.LeCongTemplate.Test.Base.TestData;
...
namespace LeCongCompany.LeCongTemplate.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="LeCongTemplateDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(LeCongTemplateDbContext context)
            {
        ...
        protected void UsingDbContext(Action<LeCongTemplateDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LeCongTemplateDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<LeCongTemplateDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<LeCongTemplateDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LeCongTemplateDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LeCongTemplateDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LeCongTemplateDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LeCongTemplateDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<LeCongTemplateDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<LeCongTemplateDbContext, Task<TResult>> func)
        {
            TResult result;
