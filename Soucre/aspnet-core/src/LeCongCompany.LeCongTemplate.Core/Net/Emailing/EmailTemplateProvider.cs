...
namespace LeCongCompany.LeCongTemplate.Net.Emailing
{
    public class EmailTemplateProvider : IEmailTemplateProvider, ISingletonDependency
    {
        ...
        public string GetDefaultTemplate(int? tenantId)
        {
            ...
            {
                using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("LeCongCompany.LeCongTemplate.Net.Emailing.EmailTemplates.default.html"))
                {
