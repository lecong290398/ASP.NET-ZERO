...
namespace LeCongCompany.LeCongTemplate.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : LeCongTemplateServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("LeCongCompany.LeCongTemplate", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
