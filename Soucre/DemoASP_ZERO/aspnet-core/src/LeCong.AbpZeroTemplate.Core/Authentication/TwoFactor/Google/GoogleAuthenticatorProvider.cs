...
namespace LeCong.AbpZeroTemplate.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : AbpZeroTemplateServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("LeCong.AbpZeroTemplate", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
